...
public class 빙고복권 extends JFrame implements ActionListener {
	...
	private void setLP(JPanel lp) {
		...
		for (HashMap.Entry<Integer, Integer> entry : map.entrySet()) {
			...
			if (cnt % 2 == 0)
				str_reward.append("<br>");
			else
				str_reward.append("&emsp;");
		}
	...
	private boolean isDiaBingo()
	{
		...
		q.clear();
		for (int i = 0; i < n; i++)
			if (!clckBtn[0 + i][ n - 1 - i].isEnabled())
				q.add(clckBtn[0 + i][ n - 1 - i]);
