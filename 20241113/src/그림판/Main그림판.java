	...
	private void addEvent() {
		...
		myPanel.setEvent(new MyPanel.MouseEv() {
			...
			public void MouseUp(Graphics g, Point start, Point end) {
				...
				if (isDiaFlag())
					...
					if (toolFlag[2]) {
						...
						oldImg = oldImgCopy(myPanel.getBufferImage(), start, end);
						...
						textbox.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								...
								g.drawImage(oldImg, start.x - 5, start.y - 5, end.x + 5, end.y + 5, 0, 0,
										Math.abs(start.x - end.x), Math.abs(start.y - end.y), null);

								int height = textbox.getBounds().height / 2;
								int penCenter = oldFont.getSize() / 2;
								g.drawString(textbox.getText(), evt_start.x + 10, evt_start.y + height + penCenter);
			...
			public void MouseDown(Graphics g, Point pt) {
				// TODO Auto-generated method stub
				if (textboxFlag) {
					g.drawImage(oldImg, evt_start.x - 5, evt_start.y - 5, evt_end.x + 5, evt_end.y + 5, 0, 0,
				...
				if (toolFlag[0]) {
					...
					if (standardColor == preColor[1].getRGB())
						return;
					Painting(bfImg, pt, preColor[1].getRGB(), standardColor);
					myPanel.Invalidate();
				} else if (toolFlag[2]) {
					useEraser(g, pt);
					myPanel.Invalidate();
