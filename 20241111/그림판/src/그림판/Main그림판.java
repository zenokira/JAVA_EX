			...
			public void MouseUp(Graphics g, Point start, Point end) {
				...
				if (isDiaFlag())
					...
					if (toolFlag[2]) {
						...
						textbox.setBounds(start.x + 5, start.y, Math.abs(start.x - end.x) - 10, Math.abs(start.y - end.y));
						

						textbox.setFont(textFont);
						textbox.setForeground(textColor);
						textbox.setText("글을 입력해주세요");
						...
						textbox.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								// TODO Auto-generated method stub
								if (e.getSource() != null && e.getSource() == textbox) {
									Color imsi_c = g.getColor();
									Font imsi_f = g.getFont();							
									g.setColor(textColor);
									g.setFont(textFont);
									
									int height = Math.abs(opt.y - end.y);
									if(textbox.getBounds().height > textFont.getSize())
									{
										height = (textbox.getBounds().height - textFont.getSize()) / 2;
									}
									g.drawString(textbox.getText(), start.x + 10,
											start.y + height / 2  );
