		...
		myPanel.setEvent(new MyPanel.MouseEv() {
			...
			public void MouseUp(Graphics g, Point start, Point end) {
				...
				if (isDiaFlag())
					...
					{
						JTextField tf = new JTextField("문자입력");
						Stroke imsi = ((Graphics2D)g).getStroke();
						
						float[]  dash = {4,2,8,2};
						((Graphics2D)g).setStroke(new BasicStroke(5,0,BasicStroke.JOIN_MITER,1.0f,dash, 0));
						g.drawRect(start.x, start.y,Math.abs(opt.x-end.x) , Math.abs(opt.y-end.y));
						tf.setBounds(start.x, start.y,Math.abs(opt.x-end.x) , Math.abs(opt.y-end.y));
						myPanel.add(tf);
					}
				}
				myPanel.Invalidate();
